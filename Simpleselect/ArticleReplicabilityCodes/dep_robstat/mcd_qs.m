function [RAW,REW,varargout] = mcd_qs(Y,varargin)
...
options=struct('ostype','sort','nsamp',nsampdef,'refsteps',refstepsdef,'bestr',bestrdef,...
    'reftol',reftoldef,...
...
% tend and the index itend are for the time monitoring
tend = zeros(nselected*10,1);  itend = 0; %#ok<SETNU>
...
if nargout==4
    varargout(1)={C};
    varargout(2)={[sum(tend(tend>0)) , std(tend(tend>0)) , sum(tend>0)]};
end
    ...
    function outIRWLS = IRWLSmcd(Y, initialloc, initialcov, h, refsteps, reftol)
        ...
        while ( (locdiff > reftol) && (iter < refsteps) )
            ...
            switch ostype
                ...
                case 'qs__'
                    t = tic;
                    mh = quickselectFS(mahaldist,h+1);
                    obs_in_set = mahaldist<mh;
                    tend(itend) = toc(t);
                case 'qso_'
                    t = tic;
                    mh = quickselectFS(mahaldist,h+1,h+1+randi(n-h-1,1,1));
                    obs_in_set = mahaldist<mh;
                    tend(itend) = toc(t);
                case 'qsc_'
                    t = tic;
                    maha1 = mahaldist(1); mdtmp = mahaldist; mdtmp(1)=maha1;
                    %SIMPLEselect(mdtmp,n,h);    %remark: h instead of h+1
                    quickselectFSmex(mdtmp,n,h); %remark: h instead of h+1
                    ksor = mdtmp(h+1);
                    obs_in_set=mahaldist<=ksor;
                    tend(itend) = toc(t);
                case 'qsoc'
                ...
                case 'nrc_'
                    t = tic;
                    maha1 = mahaldist(1); mdtmp = mahaldist; mdtmp(1)=maha1;
                    NUREselect(mdtmp,n,h); %remark: h instead of h+1
                    ksor = mdtmp(h+1);
                    obs_in_set=mahaldist<=ksor;
                    tend(itend) = toc(t);
            end
